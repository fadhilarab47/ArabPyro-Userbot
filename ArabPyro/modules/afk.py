...
from ArabPyro import BOTLOG_CHATID
from ArabPyro.helpers.msg_types import Types, get_message_type
from ArabPyro.helpers.parser import escape_markdown, mention_markdown
from ArabPyro.helpers.SQL.afk_db import get_afk, set_afk
from ArabPyro.modules.help import add_command_help
...
async def afk(client: Client, message: Message):
    if len(message.text.split()) >= 2:
        ...
        set_afk(True, "")
        await message.edit(
            "⊠ {} <b>Telah Meninggalkan Lane</b> ⊠".format(
                mention_markdown(message.from_user.id, message.from_user.first_name)
            )
        )
...
async def afk_mentioned(client: Client, message: Message):
    ...
    if get and get["afk"]:
        ...
        if get["reason"]:
            await message.reply(
                "❏ {} <b>Sedang Meninggalkan Permainan!</b>\n└ <b>Alasan:</b> <code>{}</code>".format(
                    client.me.mention, get["reason"]
                )
            )
        else:
            await message.reply(
                f"<b>Sowry</b> {client.me.first_name} <b>Sedang AFK!</b>"
            )
...
async def no_longer_afk(client: Client, message: Message):
    ...
    if get and get["afk"]:
        set_afk(False, "")
        try:
            await client.send_message(BOTLOG_CHATID, "Anda sudah kembali dari AFK!")
        except BaseException:
            pass
