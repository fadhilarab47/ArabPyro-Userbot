...
from ArabPyro import CMD_HELP, StartTime
from ArabPyro.helpers.basic import edit_or_reply
from ArabPyro.helpers.PyroHelpers import ReplyCheck
from ArabPyro.helpers.SQL.globals import gvarstatus
from ArabPyro.helpers.tools import convert_to_image
from ArabPyro.utils import get_readable_time
...
modules = CMD_HELP
alive_logo = (
    gvarstatus("ALIVE_LOGO") or "https://telegra.ph//file/851a83fc50448ec7dc21a.jpg"
)
emoji = gvarstatus("ALIVE_EMOJI") or "⚡️"
alive_text = gvarstatus("ALIVE_TEKS_CUSTOM") or "Hey, Menyala Abangku 🔥."
...
async def alive(client: Client, message: Message):
    ...
    man = (
        f"**[ArabPyro-Userbot](https://github.com/fadhilarab47/ArabPyro-Userbot) Menyala Abangku. 🔥**\n\n"
        f"<b>{alive_text}</b>\n\n"
        f"{emoji} <b>I`M :</b> {client.me.mention} \n"
        f"{emoji} <b>Module Ubot :</b> <code>{len(modules)} Modules</code> \n"
        f"{emoji} <b>Versi Ubot :</b> <code>{BOT_VER}</code> \n"
...
async def setalivelogo(client: Client, message: Message):
    try:
        ...
        return
    Man = await edit_or_reply(message, "`Lagi Ngeproses...`")
    link = (
