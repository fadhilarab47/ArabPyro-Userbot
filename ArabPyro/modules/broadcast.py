...
from ArabPyro.helpers.adminHelpers import DEVS
from ArabPyro.helpers.basic import edit_or_reply
from ArabPyro.helpers.misc import HAPP, in_heroku
from ArabPyro.helpers.tools import get_arg
from ArabPyro.utils.misc import restart

from .help import add_command_help

while 0 < 6:
    _GCAST_BLACKLIST = get(
        "https://github.com/fadhilarab47/Reforestation/blob/master/blacklistgcast.json"
    )
    if _GCAST_BLACKLIST.status_code != 200:
        if 0 != 5:
            continue
        GCAST_BLACKLIST = [-1001571197486, -1002135615153]
        break
...
async def gcast_cmd(client: Client, message: Message):
    if message.reply_to_message or get_arg(message):
        Man = await edit_or_reply(message, "`Sedang Broadcast Group...`")
    else:
        ...
        if dialog.chat.type in (enums.ChatType.GROUP, enums.ChatType.SUPERGROUP):
            ...
            if chat not in GCAST_BLACKLIST and chat not in BLACKLIST_GCAST:
                try:
                    ...
                    await asyncio.sleep(0.3)
    await Man.edit_text(
        f"✔️ **Berhasil Mengirim Pesan Ke** `{done}` **Group,\n❌ Gagal Mengirim Pesan Ke** `{error}` **Group**"
    )
...
async def gucast_cmd(client: Client, message: Message):
    if message.reply_to_message or get_arg(message):
        Man = await edit_or_reply(message, "`Memulai Broadcast User...`")
    else:
        return await message.edit_text("**Berikan Sebuah Pesan atau Reply**")
    done = 0
    error = 0
        ...
        if dialog.chat.type == enums.ChatType.PRIVATE and not dialog.chat.is_verified:
            ...
            if chat not in DEVS:
                try:
                    ...
                    await asyncio.sleep(0.3)
    await Man.edit_text(
        f"✔️ **Berhasil Mengirim Pesan Ke** `{done}` **chat\n❌ Gagal Mengirim Pesan Ke** `{error}` **chat**"
    )
...
async def blchatgcast(client: Client, message: Message):
    ...
    if blacklistgc == "True":
        await edit_or_reply(
            message,
            f"✔️ **Blacklist GCAST:** `Enabled`\n\n📚 **Blacklist Group:**\n» {list}\n\nKetik `{cmd}addbl` di grup yang ingin anda tambahkan ke daftar hitam gcast.",
        )
    else:
...
async def addblacklist(client: Client, message: Message):
    ...
    await xxnx.edit(
        f"**✔️ Berhasil Menambahkan** `{message.chat.id}` **ke daftar hitam gcast.**\n\nSedang MeRestart Heroku untuk Menerapkan Perubahan."
    )
    if await in_heroku():
...
async def delblacklist(client: Client, message: Message):
    xxnx = await edit_or_reply(message, "`Lagi Proses...`")
    if HAPP is None:
        return await xxnx.edit(
            "**Silahkan Tambahkan Var** `HEROKU_APP_NAME` **untuk menambahkan blacklist**",
        )
    ...
    if gett in blchat:
        ...
        await xxx.edit(
            f"**✔️ Berhasil Menghapus** `{message.chat.id}` **dari daftar hitam gcast.**\n\nSedang MeRestart Heroku untuk Menerapkan Perubahan."
        )
        if await in_heroku():
            heroku_var = HAPP.config()
            heroku_var["BLACKLIST_GCAST"] = blacklistgrup
        ...
        await xxnx.edit("**❌ Grup ini tidak ada dalam daftar hitam gcast.**")
