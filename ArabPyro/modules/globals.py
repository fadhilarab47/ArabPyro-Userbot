...
from ArabPyro import *
from ArabPyro.helpers.adminHelpers import DEVS, WHITELIST
from ArabPyro.helpers.basic import edit_or_reply
from ArabPyro.helpers.PyroHelpers import get_ub_chats
from ArabPyro.utils import extract_user, extract_user_and_reason
...
def globals_init():
    try:
        ...
        sql = import_module("ArabPyro.helpers.SQL.gban_sql")
        sql2 = import_module("ArabPyro.helpers.SQL.gmute_sql")
...
async def gban_user(client: Client, message: Message):
    ...
    if user_id in WHITELIST:
        return await Man.edit(
            "**Gagal GBAN karena dia adalah admin @SiArab_Support 🗿**"
        )
    if user_id:
        try:
    ...
    if sql.is_gbanned(user.id):
        return await Man.edit(
            f"[Sampah Tele](tg://user?id={user.id}) **ini sudah ada di daftar gbanned**"
        )
...
async def gmute_user(client: Client, message: Message):
    ...
    if user.id in WHITELIST:
        return await Man.edit(
            "**Gagal GMUTE karena dia adalah admin @SiArab_Support 🗿**"
        )
    try:
