...
DEF_UNAPPROVED_MSG = (
    ...
    "     ✇ 𝗪𝗔𝗥𝗡𝗜𝗡𝗚 𝗖𝗢𝗞𝗞 ✇\n"
    "╚════════════════════╝\n"
    "• Saya belum menyetujui anda untuk PC.\n"
    "• Tunggu sampai saya menyetujui PC anda.\n"
    ...
    if message.chat.id in DEVS:
        try:
            ...
            await client.send_message(
                message.chat.id,
                f"<b>Menerima Pesan!!!</b>\n{message.from_user.mention} <b>Terdeteksi Developer ArabPyro-Userbot</b>",
...
async def approvepm(client: Client, message: Message):
    ...
    try:
        approve(uid)
        await message.edit(f"**Oke Pesan Di Terima Dari** [{name0}](tg://user?id={uid})!")
    except IntegrityError:
        await message.edit(
            f"[{name0}](tg://user?id={uid}) Mungkin User Ini Sudah Di Acc."
        )
        return
...
async def disapprovepm(client: Client, message: Message):
    ...
    await message.edit(
        f"**Pesan** [{name0}](tg://user?id={uid}) **Anda Telah Di Tolak, Jangan Nyepam Mek!**"
    )
