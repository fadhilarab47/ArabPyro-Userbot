...
from ArabPyro import BOTLOG_CHATID, LOGGER
from ArabPyro.helpers.basic import edit_or_reply
from ArabPyro.helpers.misc import HAPP
...
async def restart_bot(_, message: Message):
    ...
    if HAPP is not None:
        ...
        args = [sys.executable, "-m", "ArabPyro"]
...
async def shutdown_bot(client: Client, message: Message):
    if BOTLOG_CHATID:
        await client.send_message(
            BOTLOG_CHATID,
            "**#SHUTDOWN** \n"
            "**ArabPyro-Userbot** telah di matikan!\nJika ingin menghidupkan kembali silahkan buka heroku",
        )
    await edit_or_reply(message, "**ArabPyro-Userbot Berhasil di matikan!**")
    if HAPP is not None:
...
async def logs_ubot(client: Client, message: Message):
    ...
    await client.send_document(
        message.chat.id,
        "Logs-Heroku.txt",
        thumb="ArabPyro/resources/logo.jpg",
